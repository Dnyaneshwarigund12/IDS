...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # WORKER THREAD
        self.network_data_queue = queue.Queue()
        ...
        self.network_log_data_queue = queue.Queue()
        ...
        # Adding Data in Rows
        self.power_button.clicked.connect(lambda: self.network_live_feed_thread())
    ...
    def network_live_feed_thread(self):
        if self.network_feed_is_running:
            from worker import stop_live_feed
            stop_live_feed()
            self.sniffer_thread.join()
        else:
            from worker import start_live_feed
            self.table.horizontalHeader().setVisible(True)
            self.sniffer_thread = threading.Thread(target=start_live_feed,
                                                   args=(self.network_data_queue,))
    ...
    def alert_check(self):
        ...
        if self.log_and_alert_is_running:
            from worker import stop_log_and_alert
            stop_log_and_alert()
            self.log_alert_thread.join()
        else:
            from worker import start_log_and_alert
            self.log_alert_thread = threading.Thread(target=start_log_and_alert,
                                                     args=(
                                                         os.name, self.log_folder_path, model,
                                                         self.network_log_data_queue,
                                                         self.alert_queue,))
            self.log_alert_thread.start()
    ...
    def check_alert_queue(self):
        while not self.network_log_data_queue.empty():
            ...
            if alert_data == "CLEAN":
                ...
                self.add_alert_to_table(risk_level=alert_data)
